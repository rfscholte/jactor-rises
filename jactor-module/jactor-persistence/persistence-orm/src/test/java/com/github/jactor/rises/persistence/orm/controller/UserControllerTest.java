...
import com.github.jactor.rises.persistence.orm.RequiredFieldsRule;
...
public class UserControllerTest {
    ...
    @Rule public RequiredFieldsRule requiredFieldsRule = RequiredFieldsRule.provideValues();
    ...
    @Test public void shouldFindUser() throws Exception {
        when(userServiceMock.findUsing(any(UserName.class))).thenAnswer(
                invocationOnMock -> Optional.of(
                        aUser()
                                .withUserName("jactor").with(aPerson().with(anAddress()))
                                .build().asDto()
                )
        );
