# funksjoner for å lette bruk av
...
precmd () {															# fun: genererer tittel pa konsollvindu (kjores for hver kommando)
		print -Pn "\033]0;$(git rev-parse --abbrev-ref HEAD 2> /dev/null)\007"						# fun: precmd
}																# fun: precmd
																# fun:
...
alias branch='git branch'													# cmd: vis lokale feature brancher til git repo
alias co='git checkout'														# cmd: chekcout featurebranch (fra git server hvis --track origin -b er med, eller switch til lokal branch)
alias fetch='git fetch -p'													# cmd: fetch med prune fra git server
...
alias hbui='cd $WS/hjemme/hjemme/hjemme-build'											# navigasjon: hjemme-build
...
alias hpb='cd $WS/hjemme/hjemme-persistence/hjemme-persistence-build'								# navigasjon: hjemme-persistence-build
alias hpe='cd $WS/hjemme/hjemme-persistence'											# navigasjon: hjemme-persistence
...
alias hut='cd $WS/hjemme/utils'													# navigasjon: utils
...
alias mda.det='mda | grep -i -e warning -e error'										# cmd: analyser avhengigheter i pom.xml (maven) og filtrerer bare advarsler og feil
...
alias cmd='less $HOME/.zshrc | grep "# cmd:"'											# cmd: list alle alias for kommandoer i .zshrc
...
alias con='less $HOME/.zshrc | grep -e "# var:" -e "# opt:"'									# cmd: list all configurasjon i .zshrc
...
alias fun='less $HOME/.zshrc | grep -v fun= | grep "# fun:"'									# cmd: list alle funksjonser (de som har kommentarene 'fun:') i .zshrc
...
alias nav='less $HOME/.zshrc | grep "# navigasjon:"'										# cmd: vis navigasjoner i .zshrc
